...
const EmptyChart = ({ title }) => (
  <div className="flex items-center justify-center w-full h-64 bg-base-100 p-4 rounded-2xl shadow-md">
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        if (!revenueDate || !revenueDate.resultGroupedByYearMonth) {
          setDashboardData((prev) => ({
            ...prev,
            backendMsg: false,
          }));
        ...
        const currMonthData = revenueDate.resultGroupedByYearMonth.find(
          (revenueData) => revenueData.year_month === getCurrentYearMonth()
        );

        const lastMonthData = revenueDate.resultGroupedByYearMonth.find(
          (revenueData) => revenueData.year_month === getLastYearMonth()
        );

        // Fallback values if data not found
        const currMonthRevenue = currMonthData
          ? Math.floor(currMonthData.total_amount)
          : 0;

        const lastMonthRevenue = lastMonthData
          ? Math.floor(lastMonthData.total_amount)
          : 0;

        const currMonthParti = currMonthData ? currMonthData.total_count : 0;

        const lastMonthParti = lastMonthData ? lastMonthData.total_count : 0;
        ...
        setDashboardData((prevState) => ({
          ...
          yearData: revenueDate.resultGroupedByYearMonth || [],
          programNames_Amount:
  ...
  if (
    !dashboardData.backendMsg ||
    dashboardData.yearData.length === 0 ||
    dashboardData.programNames_Amount.length === 0
  ) {
    return (
      <div className="flex flex-col items-center w-full">
        <h2 className="text-2xl font-bold mb-4 mt-4 underline text-gray-400">
  ...
  return (
    <>
      <div className="flex flex-col items-center w-full">
        ...
        {/* Consider adding null checks or using EmptyChart for specific sections where data might be missing */}
      </div>
    </>
  );
};
